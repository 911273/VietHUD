 ...
 */
/*
 * Trimmed local copy for radar_car: upstream espressif/esp-iot-solution
 * components/display/lcd/esp_lcd_axs15231b bundles a touch driver in the
 * same file, which pulls in the separate `esp_lcd_touch` managed component.
 * PlatformIO's Arduino-as-IDF-component build doesn't fetch managed
 * components automatically, and this project already has its own touch
 * driver (lib/AXS15231B_Touch), so the touch half (esp_lcd_touch_new_i2c_axs15231b
 * and everything it needs) has been removed from this .c and its .h.
 * LCD panel functionality (esp_lcd_new_panel_axs15231b) is unmodified.
 */
...
// Normally generated from idf_component.yml by the ESP-IDF component
// manager; defined by hand since this is vendored as a plain PlatformIO lib.
...
// Touch forward declarations removed — see note near top of this file.
...
static const axs15231b_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    // Upstream default sequence ends with {0x22, ..., 200} — MIPI DCS
    // ALLPOFF (0x22, "All Pixels Off"), which forces the whole panel to
    // solid black regardless of GRAM content (a manufacturing self-test
    // mode), with nothing afterward to exit it. Dropped: it left the panel
    // permanently black (backlight lit, but nothing ever drawn showed)
    // on real hardware 2026-09-14, exactly matching ALLPOFF's documented
    // behavior. See docs/V1.2_hardening_proposal.md.
};
